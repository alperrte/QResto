...
public class TableQrCodeService {
    ...
    public TableQrCodeResponse regenerateQrCode(Long tableId) {
        ...
        qrRepository.findByRestaurantTableIdAndActiveTrue(tableId)
                .ifPresent(qr -> {
                    qr.setActive(false);
                    qrRepository.save(qr);
                });

        int newVersion = qrRepository.findByRestaurantTableId(tableId).size() + 1;

        return createNewQr(table, newVersion);
    }
    ...
    public byte[] getQrImage(String qrToken) {
        ...
        String qrContent = "http://localhost:3000/qr/scan?token=" + qr.getQrToken();
